...
- `iop-vuln-metadata-download.path` - watches the manual files under `/var/lib/foreman/` (the default manual directory) for changes (air-gapped mode)
...
For disconnected installs, the metadata can be supplied manually by dropping the file into `/var/lib/foreman/` (or to location specified by `iop_vuln_metadata_downloader_manual_dir`).
...
| Gateway client key | `private/iop-core-gateway-client.key` |
| CA | `certs/ca.crt` |
