...
- **Host processes** use the published port (`127.0.0.1:5432`). `login_host: 127.0.0.1` in database roles follows this path.
...
Ansible connectivity checks run **from the host**, so they use `database_host` (often `127.0.0.1` for internal mode via the published port, or the remote hostname for external mode). Container connection strings use `postgresql` or the external hostname, not the Ansible check host, except where noted above for FDW.
