...
| `--foreman-trusted-proxy-add` | Add an IPv4/IPv6 address or CIDR that Foreman trusts for `X-Forwarded-For` (for example a Foreman Proxy or load balancer in front of Foreman). Invalid hostnames and malformed values are rejected at the CLI. May be specified multiple times; values are persisted in `parameters.yaml`. Localhost ranges (`127.0.0.0/8` and `::1`) are always included in Foreman settings and are not controlled by this flag. Example: `--foreman-trusted-proxy-add 10.10.10.20`. | `--foreman-trusted-proxies` |
| `--foreman-trusted-proxy-remove` | Remove an IPv4/IPv6 address or CIDR from the persisted trusted proxy list (same format as `--foreman-trusted-proxy-add`). May be specified multiple times. Does not remove localhost ranges or addresses added from registered Smart Proxies during deploy. | |
| `--reset-foreman-trusted-proxy-add` | Clear the entire persisted trusted proxy list for this deployment (Obsah reset flag; hidden from the main option list—see `foremanctl deploy --help` epilog). Use with repeated `--foreman-trusted-proxy-add` to replace the list, for example: `foremanctl deploy --reset-foreman-trusted-proxy-add --foreman-trusted-proxy-add 10.0.0.1 --foreman-trusted-proxy-add 10.0.0.2`. | |
