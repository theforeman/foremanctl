...
| CLI flag | Ansible variable | What it controls |
| -------- | ---------------- | ---------------- |
| `--certificate-algorithm` | `certificates_algorithm_type` | Key algorithm: `RSA` (default), `ECC` or `ML-DSA-87` |
| `--certificate-algorithm-rsa-size` | `certificates_algorithm_size` | Key size, RSA only (default 4096) |
| `--certificate-algorithm-ecc-curve` | `certificates_algorithm_curve` | Elliptic curve, ECC only (default `secp384r1`) |
...
foremanctl deploy --certificate-algorithm=ECC --certificate-algorithm-ecc-curve=secp521r1

# Generate an ML-DSA certificate set
foremanctl deploy --certificate-algorithm=ML-DSA-87
```
