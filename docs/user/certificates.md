...
Certificates generated by foremanctl use RSA keys by default. A single algorithm applies to the whole certificate set: the CA, and the server and client certificates of every hostname.

| CLI flag | Ansible variable | What it controls |
| -------- | ---------------- | ---------------- |
| `--certificate-algorithm` | `certificates_algorithm_type` | Key algorithm: `RSA` (default), `ECC`, `ML-DSA-44`, `ML-DSA-65` or `ML-DSA-87` |
| `--certificate-algorithm-size` | `certificates_algorithm_size` | Key size, RSA only (default 4096) |
| `--certificate-algorithm-curve` | `certificates_algorithm_curve` | Elliptic curve, ECC only (default `secp384r1`) |

```bash
# Generate an ECC certificate set on the secp384r1 curve
foremanctl deploy --certificate-algorithm=ECC

# Generate an ECC certificate set on a different curve
foremanctl deploy --certificate-algorithm=ECC --certificate-algorithm-curve=secp521r1

# Generate an ML-DSA certificate set
foremanctl deploy --certificate-algorithm=ML-DSA-65
```

Certificates with non-RSA keys only carry the `digitalSignature` key usage, because those keys cannot perform key encipherment. RSA certificates keep both `digitalSignature` and `keyEncipherment`.

> [!NOTE]
> The `ML-DSA-44`, `ML-DSA-65` and `ML-DSA-87` algorithms need `community.crypto` 3.3.0 to generate the keys and `python-cryptography` 49.0.0 to sign certificates and CSRs with them. CentOS Stream 10 ships a suitable `python-cryptography`; CentOS Stream 9 ships 36.0.1, on which selecting an ML-DSA algorithm fails at key generation. Candlepin is a further caveat: it is handed this CA and its legacy crypto scheme signs with `SHA256withRSA`, so a deployment that runs Candlepin still needs an RSA CA.
...
└── vars/main.yml              # Per-algorithm key parameters and key usage
...
1. **CA Generation** (when `certificates_ca: true`):
   ...
   - Generate the CA private key with the configured algorithm (`certificates_algorithm_type`, default RSA 4096)
...
2. **Host Certificate Issuance** (for each hostname in `certificates_hostnames`):
   - Generate server key and CSR with SANs (hostname plus `certificates_server_aliases`) and the key usage matching the algorithm
...
- Key algorithm: RSA 4096 by default (`certificates_algorithm_type`); `ECC` uses `certificates_algorithm_curve`
...
- Key usage: `digitalSignature`, plus `keyEncipherment` for RSA only
