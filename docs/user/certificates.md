...
| CLI flag | Ansible variable | What it controls |
| -------- | ---------------- | ---------------- |
| `--certificate-algorithm` | `certificates_algorithm_type` | Key algorithm: `RSA` (default) or `ECC` |
...
> [!NOTE]
> `ML-DSA` is not offered yet: neither the `python-cryptography` versions shipped in RHEL nor `community.crypto` can generate ML-DSA keys. The certificates role already carries its key parameters, so the option can be added once key generation works. Use `ECC` for non-RSA deployments in the meantime.
