...
| `--tar-volume-size SIZE` | Split compressed Pulp content archives into volumes of specified size (e.g., `5G`, `100M`). Creates multi-volume compressed archives named `pulp-content.tar.gz.part000000`, `.part000001`, etc. Provides both compression benefits AND splitting for media with size limits (DVDs, USB drives, FAT32 filesystems). See `man split` for SIZE format. |
...
├── pulp-content.tar.gz.part000000  (5GB compressed data)
├── pulp-content.tar.gz.part000001  (5GB compressed data)
├── pulp-content.tar.gz.part000002  (3.2GB compressed data)
└── ...
```

**How it works:**
1. Pulp content is compressed with gzip (compression ratio varies by content type)
...
- Fit on size-limited media (DVDs, USB drives, FAT32 filesystems with 4GB file limit)
- Parallel transfer across multiple media for air-gapped environments
...
# - Split into part000000 (30GB compressed stream chunk 1)
...
**How it works:**
1. Restore automatically detects `.part000000`, `.part000001`, etc. files in the backup directory
2. All volume files are concatenated in order
3. The combined compressed stream is piped to tar for extraction
4. Content is decompressed and extracted to `/var/lib/pulp`

**Automatic validation:**
The restore command validates multi-volume backups before extraction:
- **Volume count check:** Verifies the number of `.part*` files matches the expected count from backup metadata
- **Sequential numbering check:** Ensures volume files are numbered 0, 1, 2, 3... with no gaps
- Missing or out-of-sequence volume files will cause restore to fail with a detailed error message

**Air-gapped restore workflow:**
```bash
# Copy all volume files from multiple USB drives to backup directory
cp /mnt/usb1/pulp-content.tar.gz.part000000 /var/backup/foreman-backup-20260629T120000/
cp /mnt/usb2/pulp-content.tar.gz.part000001 /var/backup/foreman-backup-20260629T120000/
cp /mnt/usb3/pulp-content.tar.gz.part000002 /var/backup/foreman-backup-20260629T120000/

# Restore (auto-detects and validates all parts)
foremanctl restore /var/backup/foreman-backup-20260629T120000
```
