...
Foremanctl releases are locked to specific Foreman images with version-compatible dependencies and plugins. Over time, the contents of a particular Foreman image may be updated to reflect newer Foreman patches. This update process first updates your system to the latest foremanctl Z stream (if applicable), then instructs foremanctl to pull the latest container images. This procedure will never migrate your Foreman container beyond Z stream patches.
