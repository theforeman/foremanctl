...
Over time, the contents of a particular Foreman image may be updated to reflect newer Foreman patches.
The update process first updates your system to the latest foremanctl z-stream (if applicable), then instructs foremanctl to pull the latest container images.
This procedure will never migrate your Foreman container beyond z-stream patches.

Scroll to the correct procedure below which matches your installation environment type ([RPM](#updating-foremanctl-from-an-connected-rpm-install) and [disconnected RPM](#updating-foremanctl-from-disconnected-rpm-install)).
...
2. Consider backing up your Foreman environment before update.
  See [Backup and Restore](backup-restore.md).
3. Update all packages to their latest versions:
    - `dnf upgrade`
    - There may not be a new release available for foremanctl; that is okay.
      Foreman images are updated through step 4.
